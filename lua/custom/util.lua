...
M.bootstrap_project = function()
  ...
  local function ask_git_init(path)
    vim.ui.select({ 'Yes', 'No' }, { prompt = ' Initialize a Git repository?' }, function(choice)
      if choice == 'Yes' then
        vim.defer_fn(function()
          if vim.fn.isdirectory(path) == 1 then
            local cmd = string.format("cd %s && git init -q && git add . && git commit -m 'Initial commit' >/dev/null 2>&1", path)
            os.execute(cmd)
  ...
  local function choose_framework(category, dir, name, selected)
    ...
    if category.name:find 'Python' then
      run_in_terminal(dir, selected.cmd:gsub('$project_name', name), string.format("  Creating Python project '%s' using %s...", name, selected.name))
      finalize_project()
      return
    end
    ...
    if selected.name:find 'Spring Boot' then
      vim.ui.select({ 'Maven', 'Gradle (Kotlin)', 'Gradle (Groovy)' }, { prompt = '  Choose a build tool:' }, function(build_tool)
    ...
    if selected.name:find 'Laravel' then
      ...
      vim.ui.select(
        ...
        function(kit_choice)
          ...
          run_in_terminal(dir, selected.cmd:gsub('$project_name', name), "  Creating Laravel project '" .. name .. "'...")
          vim.defer_fn(function()
            if kit and kit.cmd ~= '' then
              run_in_terminal(target_path, kit.cmd .. ' && npm install && npm run build', 'Installing Laravel Starter Kit: ' .. kit_choice)
            end
            finalize_project(5000)
          end, 4000)
        end
    ...
    -- Default
    run_in_terminal(dir, selected.cmd:gsub('$project_name', name), string.format("  Creating %s project '%s'...", selected.name, name))
    finalize_project()
  end
